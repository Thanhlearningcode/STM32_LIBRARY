...
void RTC_init(void)
{
	RCC->APB1ENR |= (0x3<<27);//enable the power and backup interface clocks by setting the PWREN and BKPEN bitsin the RCC_APB1ENR register
	PWR->CR |= (1U<<8);//set the DBP bit the Power Control register (PWR_CR) to enable access to the Backupregisters and RTC.
	RCC->BDCR |= (1U<<0);// set LSE
	while(((RCC->BDCR >> 1U) & 0x01) != 1U){}
	RCC->BDCR &= ~(3U<<8);
	RCC->BDCR |= (1U<<8);// LSE used as RTC Clock
	RCC->BDCR |= (1U<<15); // enable RSE
